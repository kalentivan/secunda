...
usage() {
  ...
  echo
  echo "Опции:"
  echo "  -f <folder>     Папка для проекта (по умолчанию: 'meetings')"
  echo "  -r <repo>       Ссылка на репозиторий (по умолчанию: 'https://gitlab...')"
  echo "  -n <net>        Название Docker-сети (по умолчанию: 'meeting-net')"
  echo "  -i <image>      Название Docker-образа (по умолчанию: 'meeting-backend')"
  echo "  -e <env>        Путь к .env-файлу на сервере (по умолчанию: '/root/settings/meetings.env')"
  echo "  -b <branch>     Ветка Git (по умолчанию: 'dev')"
  echo "  -h              Показать эту справку и выйти"
  echo "  -s <y|n>        Установить или пропустить установку Docker (по умолчанию: 'off')"
